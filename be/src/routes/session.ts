...
function buildRelayer() {
  ...
  const publicClient = createPublicClient({ chain: MANTLE_SEPOLIA, transport });
  const walletClient = createWalletClient({ account, chain: MANTLE_SEPOLIA, transport });
...
router.post('/create', async (req: Request, res: Response) => {
  try {
    ...
    const message = `Authorize session key ${sessionKeyAddress} for MantleTap until ${expiresAt}`;
...
router.get('/status', async (req: Request, res: Response) => {
  try {
    ...
    const [usdcBalance, mntBalance] = await Promise.all([
